...
Approvals can appear during automated runs, Chat Mode write actions, public run flows, and workflow gates. They are especially useful when an agent is about to send information, update a system, make a decision with business impact, or continue past a manual review point.
...
|-------|------------------|-----------------|
| **Global approval mode** | The default behavior for your account or workspace settings | Ask approvals or Bypass approvals |
| **Agent approval mode** | Whether a specific agent inherits the default or overrides it | Default, Ask approvals, or Bypass approvals |
| **Tool approval policy** | What each connected tool or action is allowed to do | Always allow, Needs approval, or Blocked |
...
The Settings page contains the global policy that agents inherit by default.

From Settings, admins can manage:

- The default approval mode used when an agent is set to **Default**
- Workspace-level tool policies for connected toolkits and individual actions
- Which tools are always allowed, require approval, or are blocked
...
<Steps>
  ...
  <Step title="Set tool policies">
    In Settings, allow safe read actions, require approval for sensitive write actions, and block tools the workspace should not use.
  </Step>
...
<CardGroup cols={2}>
  <Card title="Dex and Agent Modes" icon="message" href="/agents/modes">
    See how approvals behave across Chat Mode, Operator Mode, and Public Mode.
  </Card>
